...
async function makeApiRequest(url, params = {}, config) {
    logger.debug("[API] makeApiRequest", {
        url: url.replace(/https?:\/\/[^\/\s:]+(?::\d+)?/, '[SERVER]'),
    ...
    try {
        ...
        logger.warn(`⚠️ API Request failed for ${sanitizedUrl} with params ${JSON.stringify(sanitizedParams)}:`, err.message);
        
        if (err.response?.status === 401) {
             logger.info("🔧 Detected Unauthorized (401). The provided access token might be invalid or expired.");
        }
...
async function getPlaybackStreams(item, seriesName = null, config) {
    ...
    if (!playbackInfoData?.MediaSources?.length > 0) {
        logger.warn("❌ No MediaSources found for item:", item.Name, `(${item.Id})`);
        return null;
    }
    ...
    for (const source of playbackInfoData.MediaSources) {
        try {
          ...
          qualityTitle = qualityTitle || 'Direct Play'; // Fallback title
            ...
            logger.error(`❌ Error processing MediaSource ${source.Id} for item ${item.Id}:`, error?.message || String(error));
    ...
    if (streamDetailsArray.length === 0) {
        logger.warn(`❌ No direct playable sources found for item: ${item.Name} (${item.Id})`);
        return null;
    }
...
async function getStream(idOrExternalId, config) {
    ...
    if (!config.serverUrl || !config.userId || !config.accessToken) {
        logger.error("❌ Configuration missing (serverUrl, userId, or accessToken)");
    ...
    try {
        ...
        if (!parsedId) {
            logger.error(`❌ Failed to parse input ID: ${idOrExternalId}`);
            return null;
        }
        ...
        logger.error(`❌ Unhandled error in getStream for ID ${fullIdForLog}:`, err?.message || String(err));
