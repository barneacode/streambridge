...
function parseMediaId(idOrExternalId) {
    ...
    if (parts.length === 3) {
        ...
        if (isNaN(seasonNumber) || isNaN(episodeNumber)) {
             logger.warn("❌ Invalid season/episode number in ID:", idOrExternalId);
        ...
        if (!idPart) {
            logger.warn(`❌ Missing ${prefix.toUpperCase()} ID part in ID:`, idOrExternalId);
            return null;
        }
        if (prefix === "tmdb") {
            tmdbId = idPart;
            ...
            logger.warn("❌ Unsupported prefix in ID:", prefix);
            return null;
        }
    } else if (parts.length !== 1) {
        logger.warn("❌ Unexpected ID format:", idOrExternalId);
        return null; // Unexpected format
    }

    if (baseId.startsWith("tt")) {
        if (baseId.length <= 2) {
            logger.warn("❌ Incomplete IMDb ID format:", baseId);
            return null;
        }
        imdbId = baseId;
        ...
        logger.warn("❌ Unsupported base ID format (expected tt..., tmdb..., tvdb..., or anidb...):", baseId);
        return null;
    }
...
function safeExtractMediaInfo(source, videoStream, audioStream) {
  try {
    ...
    logger.error('Media info extraction failed:', error?.message || String(error));
